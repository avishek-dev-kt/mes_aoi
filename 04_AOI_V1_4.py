...
# Configure logging
log_file_path = os.path.join(os.path.dirname(__file__), 'POST_AOI_app.log')
...
# Remove Skipped file names from skipped logs for backup function to move that file
...
def parse_xml_to_json(xml_file, json_file, log_file, model_id, json_folder1, json_folder2, skipped_log_file, skipped_log_folder):
    try:
        ...
        # Get the barcode and inspection result from the XML
        panel_barcode = root.find('.//Barcode').text if root.find('.//Barcode') is not None else None
        result = root.find('.//Result').text if root.find('.//Result') is not None else None
        print(panel_barcode, result)

        if not panel_barcode or not result:
            print("Not found in XML")
            logging.error(f"panel_barcode or result not found in {xml_file}")
            return
        ...
        if found:
            ...
            post_aoi_data = existing_data.get("post_aoi", [])

            # Initialize dictionary to hold parsed data
            parsed_data = {}

            # Extract required fields directly from root
            parsed_data['serial_no'] = root.findtext('Barcode')
            parsed_data['result'] = result
            parsed_data['time'] = root.findtext('Time')
            parsed_data['program_name'] = root.findtext('ProgramName')
            parsed_data['tester_name'] = root.findtext('TesterName')
            
            # Determine SIDE from ProgramName
            parsed_data['side'] = determine_side(parsed_data['program_name'])
            
            # Generate and assign a new pd_no
            last_pd_no = generate_pd_no(last_pd_no)
            parsed_data['pd_no'] = last_pd_no  # Add the new pd_no to parsed data

            # Only capture inspection data if result is PASS
            if result != "GOOD" and result == "PASS":
                ng_count = ng_count_log(xml_file) # Get ng count
                # Only capture the first relevant inspection data
                inspection_data = root.findall('.//InspectionData')
                if inspection_data:
                    for inspection in inspection_data:
                        parsed_data.update({
                            'panel_id': inspection.find('PanelID').text,
                            'ref_des': inspection.find('RefDes').text,
                            'comp_part_no': inspection.find('CompPartNo').text,
                            'machine_call': inspection.find('MachineCall').text,
                            'repair_status': inspection.find('RepairStatus').text,
                            'operator_call': inspection.find('OperatorCall').text,
                            'ng': ng_count
                        })
                        break  # Only take the first valid inspection data

            # Add the new parsed data to the 'post_aoi' list
            post_aoi_data.append(parsed_data)

            # Update existing data with new entries and model_id
            existing_data['post_aoi'] = post_aoi_data
...
def load_existing_json(json_file):
    ...
    try:
        ...
        # Access the 'post_aoi' list from the loaded data
        post_aoi_data = data.get('post_aoi', [])
        
        if post_aoi_data:
            last_pd_no = post_aoi_data[-1].get('pd_no', "PD0000")
        else:
            last_pd_no = "PD0000"  # Default value if there is no data in post_aoi
        
        return data, last_pd_no
    except json.JSONDecodeError:
        logging.error(f"Error decoding JSON from file {json_file}. Returning empty data.")
        return {"model_id": "", "post_aoi": []}, "PD0000"
    except Exception as e:
...
# Function to determine the side based on ProgramName
def determine_side(program_name):
    if program_name and program_name.endswith('SS.KYJOB'):
        return "BOTTOM"
    elif program_name and program_name.endswith('CS.KYJOB'):
        return "TOP"
...
def load_existing_json_2(json_file):
    if os.path.exists(json_file):
        ...
        return data, last_pd_no  # Return full data instead of just post_aoi list
    else:
        return None, "PD0000"
...
def send_to_erpnext(data, api_key, api_secret, erp_url, retries=3, delay=15, timeout=10):
    ...
    child_data = [{
        ...
        "tester_name": record.get("tester_name", ""),
        "side": record.get("side", ""),
        "pd_no": record.get("pd_no", ""),
        "ng": record.get("ng", ""),
        "panel_id": record.get("panel_id", ""),
        "ref_des": record.get("ref_des", ""),
        "comp_part_no": record.get("comp_part_no", ""),
        "machine_call": record.get("machine_call", ""),
        "repair_status": record.get("repair_status", ""),
        "operator_call": record.get("operator_call", "")
    } for record in post_aoi]
        ...
        while attempt < retries:
            try:
                if parent_name:
                    ...
                    existing_post_aoi = existing_data.get("data", {}).get("post_aoi", [])

                    # Check if the serial_no exists in the existing data
                    existing_record = next((item for item in existing_post_aoi if item["serial_no"] == record["serial_no"]), None)

                    if existing_record:
                        # Update the existing record with the new data
                        existing_record.update(record)
                        payload = {"post_aoi": existing_post_aoi}
                        response = requests.put(url, headers=headers, data=json.dumps(payload), timeout=timeout)
                    else:
                        # If no conflict, add the new record
                        existing_post_aoi.append(record)
                        payload = {"post_aoi": existing_post_aoi}
                        response = requests.put(url, headers=headers, data=json.dumps(payload), timeout=timeout)
                    ...
                    payload = {
                        "model_id": model_id,
                        "serial_no": record.get("serial_no", ""),
                        "post_aoi": [record],  # Send just the current record
                        "docstatus": 0
                    }
...
def task_workflow(api_key, api_secret, erp_url, machine_data_folder, json_folder1, json_folder2):
    ...
    # NG_log_file = get_log_file_path_2("NG_Logs")
